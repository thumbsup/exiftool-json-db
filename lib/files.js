...
exports.read = function (mediaPath, existingData) {
  ...
  findFiles((err, files) => {
    ...
    async.mapLimit(files, 100, readStats, (err, stats) => {
      if (err) return emitter.emit('error', err)
...
function findNewer (databaseMap, diskStats) {
  ...
  for (i = 0; i < diskStats.length; ++i) {
    ...
    if (!fromDatabase) {
      // console.log('Not in database: ' + filePath)
      ...
      if (Math.abs(lastModified - fromDisk.fileDate) >= 1000) {
        // console.log(`In database but updated: ${filePath} DB=${fromDatabase.File.FileModifyDate}=${lastModified} Disk=${fromDisk.fileDate}`)
        modified.push(fromDisk.relative)
      } else {
        // console.log('In database, unchanged: ' + filePath)
